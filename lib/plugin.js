...
Plugin.prototype.install = function(cb) {
  ...
  const cmd = 'pnpm add ' + this.deps.join(' ');
  log.debug(cmd);
...
Plugin.init = function(head) {
  ...
  for (let k of _.keys(stats)) {
    ...
    // a disabled plugin absent from disk was deliberately dropped (e.g. by a
    // refactor); auto-installing it from upstream would resurrect old code
    // this branch no longer ships, so skip it instead.
    if (!stats[k]) continue;
...
Plugin.copy = function(src, cb) {
  ...
  if (src.startsWith('https://')) {
    request({url: src, responseType: 'stream'}, function(e, resp, srcstream) {
      if (e) {
        dststream.emit('error', e);
      } else if (resp.statusCode !== 200) {
        dststream.emit('error', 'HTTP Error: ' + resp.statusCode);
      } else {
        srcstream.on('error', function(e2) {
          dststream.emit('error', e2);
        });
