...
core.getProblem = function(keyword, needTranslation, cb) {
  ...
  // accept a full problem url, e.g. https://leetcode.com/problems/two-sum/
  const url = /\/problems\/([^/]+)/.exec(keyword);
  if (url) keyword = url[1];
...
core.getProblemOfToday = function(needTranslation, cb) {
  this.next.getProblemOfToday(needTranslation, function(e, slug) {
    if (e) return cb(e);

    core.getProblems(needTranslation, function(e, problems) {
      if (e) return cb(e);

      const problem = problems.find(x => x.slug === slug);
      if (!problem) return cb('Problem not found!');
      core.next.getProblem(problem, needTranslation, cb);
    });
  });
...
core.getCalendar = function(cb) {
  this.next.getCalendar(cb);
};
...
// greedy word wrap at `width` columns, preserving existing newlines
// (replaces the abandoned `wordwrap` package)
