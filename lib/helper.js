...
// lodash.startCase replacement for the common snake/camel case keys
h.startCase = function(s) {
...
h.langToCommentStyle = function(lang) {
  const res = LANGS.find(x => x.lang === lang);
  if (!res) return {start: '', line: '', end: '', singleLine: ''};
...
// minimal interactive prompt on node:readline, fields: [{name, hidden}]
h.readInput = function(fields, cb) {
  ...
  (function ask(i) {
    ...
    const q = fields[i];
    const visible = rl.output.write.bind(rl.output);
    rl.question(q.name + ': ', function(answer) {
      if (q.hidden) {
        delete rl.output.write; // restore echo
        visible('\n');
      }
      result[q.name] = answer;
      ask(i + 1);
    });
    if (q.hidden) rl.output.write = function() { return true; }; // mute secrets
  })(0);
};
