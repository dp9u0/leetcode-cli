...
file.meta = function(filename) {
  ...
  for (let i = 0; i < tokens.length; ++i) {
    const v = tokens[i].split('=');
    if (v.length != 2) continue;
    // some ids are prefixed and contain a space, e.g. `id=LCR 064`; the
    // numeric part is a bare token without '=', so append following bare
    // tokens (until the next key=value) to this value.
    let val = v[1];
    while (i + 1 < tokens.length && tokens[i + 1].indexOf('=') < 0) {
      val += ' ' + tokens[++i];
    }
    m[v[0]] = val.trim();
  }
