...
const cmd = {
  ...
  builder: function(yargs) {
    return yargs
      ...
      .example(chalk.yellow('leetcode show -d'), 'Show question of the day')
...
function showProblem(problem, argv) {
  ...
  log.info();
  log.info(plainText(problem.desc));

  for (let i of (problem.hints || []).keys()) {
    log.info();
    log.printf('%s %s', chalk.yellow('Hint ' + (i + 1) + ':'), plainText(problem.hints[i]));
  }
}

// terminal-friendly text: strip tags, decode entities, keep line structure
function plainText(html) {
  return he.decode(cheerio.load('' + html).root().text()).trim();
}
