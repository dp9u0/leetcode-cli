...
cmd.handler = function(argv) {
  ...
  core.filterProblems(argv, function(e, problems) {
    ...
    // column widths follow the data: fids grow over time and some names
    // exceed 60 chars, either would shift the level column of the row
    const fidWidth = problems.length ? ('' + problems[0].fid).length : 4;
    const nameWidth = problems.reduce((m, x) => Math.max(m, x.name.length), 60);
    const extraIndent = 8 + fidWidth;
    ...
    for (let problem of problems) {
      ...
      log.printf('%s %s %s [%=' + fidWidth + 's] %-' + nameWidth + 's %-6s (%s %%)',
      ...
      if (argv.extra) {
        ...
        let len = 0;
        for (let x of badges) {
          if (len + x.length + 3 >= nameWidth) {
            log.printf('%' + extraIndent + 's%s', ' ', chalk.gray(buf.join(' | ')));
            buf = [];
            len = 0;
          }
          buf.push(x);
