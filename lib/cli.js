...
function initLogLevel() {
  ...
  if (level === 'TRACE') {
    ...
    console.error = _.wrap(console.error, function(func) {
      ...
      // args may hold non-strings (yargs logs through console.error too)
      if (args.length > 0 && typeof args[0] === 'string' &&
          args[0].indexOf('REQUEST ') === 0) {
...
function runCommand() {
  // -v/-vv were already consumed by initLogLevel; yargs snapshots
  // process.argv at require time and bails silently on unknown options
  // placed before the command, so strip them BEFORE it loads
  process.argv = process.argv.filter(x => x !== '-v' && x !== '-vv');
